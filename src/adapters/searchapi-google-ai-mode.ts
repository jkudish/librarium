...
export interface SearchApiGoogleAiModeProviderOptions
...
/**
 * The documented Google AI Mode answer envelope also includes local and web
 * SERP sections. This adapter intentionally consumes only the AI answer
 * fields so those sections can never become organic fallback content.
 */
interface SearchApiGoogleAiModeResponse extends SearchApiAiResponse {
  local_results?: unknown;
  web_results?: unknown;
  inline_shopping?: unknown;
}

/** SearchAPI-observed Google AI Mode answer. */
export class SearchApiGoogleAiModeProvider extends BaseProvider {
  readonly id = 'searchapi-google-ai-mode';
  readonly tier: ProviderTier = 'ai-grounded';
  private readonly zeroRetention: boolean;
  private readonly retry?: HttpRetryPolicy;

  constructor(options: SearchApiGoogleAiModeProviderOptions = {}) {
    super(options);
  ...
  async execute(
    ...
    try {
      ...
      const response = await this.request<SearchApiGoogleAiModeResponse>(
        request.url,
        request.options,
      );
